...
def parse_log_file(args):
    ...
    with open(filename, "r") as fp:
        for line_no, line in enumerate(fp):
            global xabr_enable_pipeline
            if "XABR pipelining enabled" in line:
                xabr_enable_pipeline = True
...
def get_latency(s_data, e_data):
    ...
    for i, frm in enumerate(e_data.keys()):
        if i == 0:
            first_frm_num = frm
        elif i == len(e_data.keys()) - 1:
            last_frm_num = frm
    return mean(diffs)/1000000.0, min(diffs)/1000000.0, max(diffs)/1000000.0, first_frm_num, last_frm_num
...
def print_latency(data,
    ...
    lat, min, max, frm_start, frm_end = get_latency(s_data, e_data)

    global total_avg_sdk_latency
    global total_min_sdk_latency
    global total_max_sdk_latency
    ...
    elif(s_accel == "DEC" and e_accel == "DEC" and s_tag == "DECODER"):
        print("{} latency (avg) = {:.3f} ms (APPLICATION LEVEL) [min = {:.3f} ms, max = {:.3f} ms]".format(log_prefix, lat, min, max))
    elif(s_accel == "ENC" and s_tag == "Encoder::SDK"):
        print("{} latency (avg) = {:.3f} ms (SDK LEVEL) [min = {:.3f} ms, max = {:.3f} ms]".format(log_prefix, lat, min, max))
        total_avg_sdk_latency += lat
        total_min_sdk_latency += min
        total_max_sdk_latency += max
    elif(s_accel == "DEC" and e_accel == "DEC" and s_tag == "Decoder::SDK"):
        print("{} latency (avg) = {:.3f} ms (SDK LEVEL) [min = {:.3f} ms, max = {:.3f} ms]".format(log_prefix, lat, min, max))
        total_avg_sdk_latency += lat
        total_min_sdk_latency += min
        total_max_sdk_latency += max
    elif(s_accel == "ABR" and e_accel == "ABR" and s_tag == "XABRSDK"):
        if(xabr_enable_pipeline):
            print("{} latency (avg) = {:.3f} ms (SDK LEVEL) [min = {:.3f} ms, max = {:.3f} ms] --> (XABR pipelining enabled, latency values may not be correct)".format(log_prefix, lat, min, max))
        else:
            print("{} latency (avg) = {:.3f} ms (SDK LEVEL) [min = {:.3f} ms, max = {:.3f} ms]".format(log_prefix, lat, min, max))
    ...
    print("\t (start_frame: {}, end_frame: {})".format(frm_start, frm_end))
...
xabr_enable_pipeline = False
total_avg_sdk_latency = 0
total_min_sdk_latency = 0
total_max_sdk_latency = 0
if __name__ == "__main__":
    ...
    #         help="Show latency for each individual components",
    #         action='store_true')    # True value when the argument is provided, sets to False when the argument is not provided
    ...
    if(xabr_enable_pipeline):
        print("Sum of SDK latency for all IPs (avg) = {:.3f} ms [min = {:.3f} ms, max = {:.3f} ms] --> (XABR pipelining enabled)\n".format(total_avg_sdk_latency, total_min_sdk_latency, total_max_sdk_latency))
    else:
        print("Sum of SDK latency for all IPs (avg) = {:.3f} ms [min = {:.3f} ms, max = {:.3f} ms]\n".format(total_avg_sdk_latency, total_min_sdk_latency, total_max_sdk_latency))
