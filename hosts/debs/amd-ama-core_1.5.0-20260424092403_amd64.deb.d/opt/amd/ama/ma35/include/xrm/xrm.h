...
#include <stdint.h>
#include <stdbool.h>
// As an alternative to the following, boost also has a UUID library.
...
typedef struct xrmCuResource {
    ...
    int32_t deviceNumBDF;     // device number BDF, Bus - bit[15, 8], Device - bit[7, 3], Function - bit[2, 0]
    uint8_t extData[60];      // for future extension
} xrmCuResource;
...
/* request compute resource property logic group information version 2 */
typedef struct xrmCuPropertyLogicGroupInfoV2 {
    uint32_t constraintInfo;            /* resource allocation logic group constraint information
                                         * bit[31 -  8] reserved
                                         * bit[ 7 -  0] constraintType
                                         *    XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_NULL
                                         *                : no specified logic group constraint
                                         *    XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_REAL_LOGIC_GROUP_ID
                                         *                : real logic group id. It's used to identify the
                                         *                  logic group id is used as the constraint.
                                         *    XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_LOGIC_GROUP_TYPE
                                         *                : logic group type. It's used to identify the logic group
                                         *                  type is used as the constraint.
                                         *    XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_PSEUDO_LOGIC_GROUP_ID
                                         *                : pseudo logic group id. it's used to descript multiple cu
                                         *                  from same logic group if logic group id is same. It does
                                         *                  not means cu from target id logic group . This type is
                                         *                  only valid in property of cu list. It's not valid for
                                         *                  single cu.
                                         *    XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_LOGIC_GROUP_TYPE_AND_PSEUDO_LOGIC_GROUP_ID
                                         *                : logic group type and pseudo logic group id. it's used to
                                         *                  identify the logic group type and pseudo logic group id
                                         *                  are used as the constraint. This type is
                                         *                  only valid in property of cu list. It's not valid for
                                         *                  single cu.
                                         *    others      : reserved
                                         */
    int32_t groupId;                    /* logic group id, only valid for constraint type:
                                         *    XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_REAL_LOGIC_GROUP_ID
                                         *
                                         *    XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_PSEUDO_LOGIC_GROUP_ID
                                         *    XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_LOGIC_GROUP_TYPE_AND_PSEUDO_LOGIC_GROUP_ID
                                         *    For pseudo logic group id:
                                         *        groupId >= 0: sub set CU with same groupId from same logic group;
                                         *                      what's more, different set from different logic group.
                                         *        groupId < 0 : sub set CU with same groupId from same logic group;
                                         *                      however, not require to allocate different set from
                                         *                      different logic group.
                                         */
    char groupType[XRM_MAX_LOGIC_GROUP_TYPE_LEN];
                                        /* logic group type (len is 16), only valid for constraint type:
                                         *    XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_LOGIC_GROUP_TYPE
                                         *    XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_LOGIC_GROUP_TYPE_AND_PSEUDO_LOGIC_GROUP_ID
                                         */
    uint8_t extData[8];                 // for future extension
} xrmCuPropertyLogicGroupInfoV2;
...
typedef struct xrmCuPropertyV2 {
    ...
    char kernelAlias[XRM_MAX_NAME_LEN]; // unique alias of kernel name
    bool devExcl;                       // request exclusive device usage for this client
    uint64_t deviceInfo;                /* resource allocation device constraint information
                                         * bit[63 - 40] reserved
                                         * bit[39 - 32] constraintType
                                         *    XRM_DEVICE_INFO_CONSTRAINT_TYPE_NULL
                                         *                : no specified device constraint
                                         *    XRM_DEVICE_INFO_CONSTRAINT_TYPE_HARDWARE_DEVICE_INDEX
                                         *                : hardware device index. It's used to identify hardware
                                         *                  device index is used as the constraint.
                                         *    XRM_DEVICE_INFO_CONSTRAINT_TYPE_VIRTUAL_DEVICE_INDEX
                                         *                : virtual device index. it's used to descript multiple cu
                                         ...
                                         */
    uint64_t memoryInfo;                /* resource allocation memory constraint information
                                         * bit[63 - 40] reserved
                                         * bit[39 - 32] constraintType
                                         *    XRM_MEMORY_INFO_CONSTRAINT_TYPE_NULL
                                         *                : no specified memory constraint
                                         *    XRM_MEMORY_INFO_CONSTRAINT_TYPE_HARDWARE_MEMORY_INDEX
                                         *                : hardware memory bank. It's used to identify hardware
    ...
    uint64_t policyInfo;                /* resource allocation policy information
                                         * bit[63 -  8] reserved
                                         * bit[ 7 -  0] policyType
                                         *    XRM_POLICY_INFO_CONSTRAINT_TYPE_NULL
                                         *                : no specified policy
                                         *    XRM_POLICY_INFO_CONSTRAINT_TYPE_CU_MOST_USED_FIRST
                                         *                : cu most used first
    ...
    uint64_t poolId;                    /* request to allocate resource from specified resource pool,
                                         * 0 means to allocate resource from system default resource pool.
                                         */
    xrmCuPropertyLogicGroupInfoV2 logicGroupInfo; // logic group information
    uint8_t extData[32];                // for future extension
} xrmCuPropertyV2;
...
#define XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_SHIFT 0
#define XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_MASK 0xFF

#define XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_NULL 0x0
#define XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_REAL_LOGIC_GROUP_ID 0x1
#define XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_LOGIC_GROUP_TYPE 0x2
#define XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_PSEUDO_LOGIC_GROUP_ID 0x3
#define XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_LOGIC_GROUP_TYPE_AND_PSEUDO_LOGIC_GROUP_ID 0x4
...
typedef struct xrmUdfCuPropertyV2 {
    ...
    uint64_t deviceInfo;           /* resource allocation device constraint information
                                    ...
                                    * bit[31 -  0] deviceIndex
                                    */
    uint64_t memoryInfo;           /* memory constraint information
                                    * bit[63 - 40] reserved
                                    * bit[39 - 32] constraintType
...
/* compute resource logic group information version 2 */
typedef struct xrmCuResourceLogicGroupInfoV2 {
    uint32_t indicator;       /* logic group indicator:
                               * bit[31 - 8] reserved
                               * bit[ 7 - 0] indicate whether following logic group id / type is valid
                               *    XRM_LOGIC_GROUP_INFO_INVALID
                               *             : logic group id / type is invalid
                               *    XRM_LOGIC_GROUP_INFO_ID_TYPE_VALID
                               *             : logic group id / type is invalid
                               *    others   : reserved
                               */
    uint32_t groupId;         // logic group id, valid if indicator is XRM_LOGIC_GROUP_INFO_ID_TYPE_VALID.
    char groupType[XRM_MAX_LOGIC_GROUP_TYPE_LEN];
                              // logic group type (len is 16), valid if indicator is XRM_LOGIC_GROUP_INFO_ID_TYPE_VALID.
    uint8_t extData[8];       // for future extension
} xrmCuResourceLogicGroupInfoV2;
...
typedef struct xrmCuResourceV2 {
    ...
    uint64_t poolId;          // id of the cu pool this cu comes from, default pool id is 0
    int32_t deviceNumBDF;     // device number BDF, Bus - bit[15, 8], Device - bit[7, 3], Function - bit[2, 0]
    xrmCuResourceLogicGroupInfoV2 logicGroupInfor; // logic group information
    uint8_t extData[28];      // for future extension
} xrmCuResourceV2;

/* logic group information */
#define XRM_LOGIC_GROUP_INFO_INDICATOR_SHIFT 0
#define XRM_LOGIC_GROUP_INFO_INDICATOR_MASK 0xFF

#define XRM_LOGIC_GROUP_INFO_INVALID 0x0
#define XRM_LOGIC_GROUP_INFO_ID_TYPE_VALID 0x1
...
typedef int32_t (*xrmPluginVersionType)(void);
typedef int32_t (*xrmApiVersionType)(void);
 ...
 *             deviceNumBDF: device number BDF of this cu.
 * @return int32_t, 0 on success or appropriate error number
 */
 ...
 *             kernelAlias: the alias of kernel name requested.
 *             devExcl: request exclusive device usage for this client.
 *             deviceInfo: resource allocation device constraint information.
 *                         bit[63 - 40] reserved
 *                         bit[39 - 32] constraintType
 *                            XRM_DEVICE_INFO_CONSTRAINT_TYPE_NULL
 *                                      : no specified device constraint
 ...
 *                            others    : reserved
 *                         bit[31 -  0] deviceIndex
 *             memoryInfo: resource allocation memory constraint information.
 *                         bit[63 - 40] reserved
 *                         bit[39 - 32] constraintType
 *                            XRM_MEMORY_INFO_CONSTRAINT_TYPE_NULL
 *                                      : no specified memory constraint
 ...
 *                                      : no specified policy
 *                            XRM_POLICY_INFO_CONSTRAINT_TYPE_CU_MOST_USED_FIRST
 *                                      : cu most used first, only apply to cu allocation not cu list/group allocation
 *                            XRM_POLICY_INFO_CONSTRAINT_TYPE_CU_LEAST_USED_FIRST
 *                                      : cu least used first, only apply to cu allocation not cu list/group allocation
 *                            XRM_POLICY_INFO_CONSTRAINT_TYPE_DEV_MOST_USED_FIRST
 *                                      : device most used first, only apply to cu allocation not cu list/group allocation
 *                            XRM_POLICY_INFO_CONSTRAINT_TYPE_DEV_LEAST_USED_FIRST
 *                                      : device least used first, only apply to cu allocation not cu list/group allocation
 *                            others    : reserved
 *             requestLoad: request load, only one type granularity at one time.
 *                          bit[31 - 28] reserved
 *                          bit[27 -  8] granularity of 1000000 (0 - 1000000)
 *                          bit[ 7 -  0] granularity of 100 (0 - 100)
 *             logicGroupInfo: logic group information.
 *                  constraintInfo: resource allocation logic group constraint information
 *                          bit[31 -  8] reserved
 *                          bit[ 7 -  0] constraintType
 *                             XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_NULL
 *                                       : no specified logic group constraint
 ...
 *                                          single cu.
 *                             others     : reserved
 *                   groupId              : logic group id, only valid for constraint type:
 *                                             XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_REAL_LOGIC_GROUP_ID
 *                                             XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_PSEUDO_LOGIC_GROUP_ID
 *                                             XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_LOGIC_GROUP_TYPE_AND_PSEUDO_LOGIC_GROUP_ID
 *                   groupType            : logic group type (len is 16), only valid for constraint type:
 *                                             XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_LOGIC_GROUP_TYPE
 *                                             XRM_LOGIC_GROUP_INFO_CONSTRAINT_TYPE_LOGIC_GROUP_TYPE_AND_PSEUDO_LOGIC_GROUP_ID
 *             poolId: request to allocate cu from specified resource pool.
 * @param cuRes the cu resource.
 ...
 *                      0. Allocation policy is not applied for cu list allocation, so no need to handle it.
 *
 *                      1. Device constraint is priority 1, it's handled first.
 *
 *                         To handle the device constraint information, CU list will be put into differernt
 *                         sub-list:
 *                         a) one or multiple sub-lists with same hw device index, different index into
 *                            different sub-list. CU in such sub-list will be allocated from target device later.
 *                         b) one or multiple sub-lists with same virtual device index, different index
 *                            into different sub-list. cu in same sub list will be allocated from same device.
 *                            different sub-list will be allocated from different device later.
 *                         c) one sub-list for cu without device constraint. cu in this sub-list will be
 *                            allocated from any device later.
 *
 *                      2. Logic group constraint is priority 2, it's handled based on device constraint
 *                         process result.
 *
 *                         Above device constraint based sub-lists willbe further put into logic group
 *                         constraint based small sub-lists:
 *                         a) one or multiple sub-list with same real group id.
 *                         b) one or multiple sub-list with same group type.
 *                         c) one or multiple sub-list with same pseudo group id, different list
 *                            from different logic group.
 *                         d) one or multiple sub-list with same pseudo group id and type, different list
 *                            from different logic group.
 *                         e) one sub-list for cu without logic group constraint.
 *
 *                         Then these sub logic group based on sub-lists will be allocated accordingly:
 *                         1) if logic group constraint not set: allocate it from any logic group.
 *                         2) if logic group constraint set with real group id: allocate it from target logic group.
 *                         3) if logic group constraint set with group type: allocate it from logic group with the type.
 *                         4) if logic group constraint set with pseudo group id: find all others with same
 *                            pseudo group id, put them into one sub-list, and allocate requested CU
 *                            from same logic group, the logic group id should be different from that
 *                            for previous sub-lists request from pseudo group id.
 *                         5) if logic group constraint set with pseudo group id and type: find all others with same
 *                            pseudo group id, put them into one sub-list, and check the group type information. If
 *                            group type of same sub-list is different, whole allocation will fail. Then allocate
 *                            requested CU from same logic group, the logic group id should be different from that
 *                            for previous sub-lists request from pseudo group id.
 *             cuNum: request number of cu in this list.
 * @param cuListRes the cu list resource.
 *             cuResources: cu resource list to fill the allocated cus infor, starting from cuResources[0], no hole.
 ...
 *             cuListProp: cu prop list to fill kernelName, devExcl and requestLoad etc. information.
 *                         For each cu in the list:
 *                      0. Allocation policy is not applied for cu list reservation, so no need to handle it.
 *
 *                      1. Device constraint is priority 1, it's handled first.
 *
 *                         To handle the device constraint information, CU list will be put into differernt
 *                         sub-list:
 *                         a) one or multiple sub-lists with same hw device index, different index into
 *                            different sub-list. CU in such sub-list will be reserved from target device later.
 *                         b) one or multiple sub-lists with same virtual device index, different index
 *                            into different sub-list. cu in same sub list will be reserved from same device.
 *                            different sub-list will be reserved from different device later.
 *                         c) one sub-list for cu without device constraint. cu in this sub-list will be
 *                            reserved from any device later.
 *
 *                      2. Logic group constraint is priority 2, it's handled based on device constraint
 *                         process result.
 *
 ...
 *                         e) one sub-list for cu without logic group constraint.
 *
 *                         Then these sub logic group based on sub-lists will be reserved accordingly:
 *                         1) if logic group constraint not set: reserved it from any logic group.
 *                         2) if logic group constraint set with real group id: reserve it from target logic group.
 *                         3) if logic group constraint set with group type: reserve it from logic group with the type.
 *                         4) if logic group constraint set with pseudo group id: find all others with same
 *                            pseudo group id, put them into one sub-list, and reserve requested CU
 *                            from same logic group, the logic group id should be different from that
 *                            for previous sub-lists request from pseudo group id.
 *                         5) if logic group constraint set with pseudo group id and type: find all others with same
 *                            pseudo group id, put them into one sub-list, and check the group type information. If
 *                            group type of same sub-list is different, whole reservation will fail. Then reserve
 *                            requested CU from same logic group, the logic group id should be different from that
 *                            for previous sub-lists request from pseudo group id.
 *             cuListNum: request number of such cu list for this pool.
 *             xclbinUuid: request all resource in the xclbin.
