...
XlnxParameterLookup param_log_level_lookup[PARAM_LOG_LEVEL_KEYS] = {{"emergency", XMA_EMERGENCY_LOG}, {"alert", XMA_ALERT_LOG}, {"critical", XMA_CRITICAL_LOG},
    {"error", XMA_ERROR_LOG}, {"warning", XMA_WARNING_LOG}, {"notice", XMA_NOTICE_LOG}, {"info", XMA_INFO_LOG}, {"debug", XMA_DEBUG_LOG}};

XlnxParameterLookup param_log_location_lookup[PARAM_LOG_LOCATION_KEYS] = {
    {"none", XMA_LOG_TYPE_NONE}, {"console", XMA_LOG_TYPE_CONSOLE}, {"syslog", XMA_LOG_TYPE_SYSLOG}, {"file", XMA_LOG_TYPE_FILE}};
...
bool xlnx_utils_check_if_pattern_matches(char* pattern, char* str) {
  ...
  if (ret) {
    ...
    printf("Error compiling regex using pattern \"%s\"!\n", pattern);
    exit(XMA_APP_ERROR);
  }
...
/**
 * xlnx_utils_key_from_string: maps string input to the parameter value
 *
 * @param param_lookuptable : parameter's lookup table
 * @param param_key: parameter in string format
 * @param num_keys: number of keys in the lookup table
 * @return The parameter key value or XMA_APP_ERROR on error
 */
int xlnx_utils_key_from_string(XlnxParameterLookup* param_lookuptable, char* param_key, int num_keys) {
  int cnt;
  if (param_key == NULL) {
    return XMA_APP_ERROR;
  }
  for (cnt = 0; cnt < num_keys; cnt++) {
    XlnxParameterLookup lookup_key = param_lookuptable[cnt];
    if (strcmp(lookup_key.key, param_key) == 0) {
...
/**
 * xlnx_utils_key_from_value: maps integer input to the parameter key
 *
 * @param param_lookuptable : parameter's lookup table
 * @param param_value: parameter in integer format
 * @param num_keys: number of keys in the lookup table
 * @return The parameter key or NULL on error
 */
char* xlnx_utils_key_from_value(XlnxParameterLookup* param_lookuptable, int param_value, int num_keys) {
  int cnt;
  for (cnt = 0; cnt < num_keys; cnt++) {
    XlnxParameterLookup lookup_key = param_lookuptable[cnt];
    if (param_value == lookup_key.value) {
 ...
 */
int xlnx_utils_set_size_t_arg(size_t* destination, char* source, char* param_name) {
  ...
  *destination = strtoumax(source, &end_ptr, 10);
  if (errno) {
    if (is_param_given) {
      ...
      printf("Unable to set param %s. %s\n", param_name, strerror(errno));
    }
    return XMA_APP_ERROR;
  }
 ...
 * avoid logging an error.
 */
int xlnx_utils_set_pix_fmt_arg(XmaFormatType* destination, char* source, char* param_name) {
  if (strcmp(source, "yuv420p") == 0) {
    *destination = XMA_YUV420P_FMT_TYPE;
  } else if (strcmp(optarg, "nv12") == 0) {
    *destination = XMA_NV12_FMT_TYPE;
  } else if (strcmp(optarg, "yuv420p10le") == 0) {
    *destination = XMA_YUV420P10LE_FMT_TYPE;
  } else if (strcmp(optarg, "p010le") == 0) {
    *destination = XMA_P010LE_FMT_TYPE;
  } else if (strcmp(optarg, "packed10") == 0) {
    *destination = XMA_PACKED10_FMT_TYPE;
  } else if (strcmp(optarg, "rgbp") == 0) {
    *destination = XMA_RGB24_P_FMT_TYPE;
  } else {
    if (strcmp(param_name, "") != 0) {
      //            xma_logmsg(XMA_ERROR_LOG, XLNX_APP_UTILS_MODULE,
      //                       "Unsupported pixel format %s option %s\n", source,
      //                       optarg);
      printf("Unsupported pixel format %s option %s\n", source, optarg);
    }
    return XMA_APP_ERROR;
  }
  return XMA_APP_SUCCESS;
...
double xlnx_utils_get_total_time(XlnxAppTimeTracker* timer) {
  xlnx_utils_set_current_time(timer);
  // in microseconds
  double time = (timer->curr_time.tv_sec - timer->start_time.tv_sec) * 1e6 + (timer->curr_time.tv_nsec - timer->start_time.tv_nsec) / 1e3;
...
double xlnx_utils_get_segment_time(XlnxAppTimeTracker* timer) {
  xlnx_utils_set_current_time(timer);
  // in microseconds
  double time = (timer->curr_time.tv_sec - timer->segment_time.tv_sec) * 1e6 + (timer->curr_time.tv_nsec - timer->segment_time.tv_nsec) / 1e3;
  return time / 1000000.0;
}
 ...
 * @return void
 */
static void xlnx_utils_extract_pixel_from_xv15_word(uint8_t pixel_index, uint32_t vcu_word, uint16_t** out_word) {
  if (pixel_index == 0) {
 ...
 * @return void
 */
static void xlnx_utils_y_xv15_wrd_10le_wrds(uint8_t num_pxls_to_xtrct, uint32_t xv15_word, uint16_t** y_plane) {
 ...
 * @return void
 */
static void xlnx_utils_uv_xv15_wrd_to_10le_wrds(uint8_t num_pxls_to_xtrct, uint32_t xv15_word1, uint32_t xv15_word2, uint16_t** u_plane, uint16_t** v_plane) {
  switch (num_pxls_to_xtrct) {
  case 6:
 ...
 */
int xlnx_utils_xv15_to_yuv420p10le(uint16_t width, uint16_t height, uint32_t xv15_linesize, uint32_t* xv15_y, uint32_t* xv15_uv, FILE* out_f) {
  ...
  for (h = 0; h < num_rows_in_plane; h++) {
    word_index = 0;
    for (w = 0; w < valid_words_in_line; w++) {
      xlnx_utils_uv_xv15_wrd_to_10le_wrds(6, xv15_uv[word_index], xv15_uv[word_index + 1], &tmp_u, &tmp_v);
      word_index += 2;
    }
    xlnx_utils_uv_xv15_wrd_to_10le_wrds(leftover_pixels, xv15_uv[word_index], xv15_uv[word_index + 1], &tmp_u, &tmp_v);
 ...
 * @return XMA_APP_SUCCESS on success, XMA_APP_ERROR on error
 */
int xlnx_utils_nv12_to_yuv420p(uint16_t width, uint16_t height, uint16_t nv12_linesize, uint8_t* nv12_y, uint8_t* nv12_uv, FILE* out_f) {
...
/**
 * Get the number of valid bytes in a line
 * @param log The handle to the logging
 * @param fprops The xma frame properties
 * @param plane The plane of the frame
 */
int xlnx_utils_get_valid_line_size(XmaLogHandle log, XmaFrameProperties* fprops, int plane) {
  ...
  case XMA_YUV422P_FMT_TYPE:
  case XMA_YUV422SP_FMT_TYPE:
  case XMA_UYVY422_FMT_TYPE:
  case XMA_YUY2422_FMT_TYPE:
    if (plane > 0) {
      return fprops->width / 2;
    } else {
      return fprops->width * 2;
  ...
  case XMA_YUV422SP_10BIT_FMT_TYPE:
    if (plane == 0) {
      return fprops->width * 2;
    } else {
      return fprops->width;
  ...
  case XMA_RGB24_P_FMT_TYPE:
    return fprops->width;
  case XMA_BGRA_FMT_TYPE:
  case XMA_ARGB_FMT_TYPE:
    return fprops->width * 4;
  case XMA_P010LE_FMT_TYPE:
    return fprops->width * 2;
  case XMA_PACKED10_FMT_TYPE:
    return ((fprops->width + 2) / 3) * 4;
  default:
    xma_logmsg(log, XMA_ERROR_LOG, XLNX_APP_UTILS_MODULE, "Pixel format not supported!\n");
    return XMA_ERROR;
  }
}

/**
 * Get the number of valid lines
 * @param log The handle to the logging
 * @param fprops The xma frame properties
 * @param plane The plane of the frame
 */
int xlnx_utils_get_valid_lines(XmaLogHandle log, XmaFrameProperties* fprops, int plane) {
  switch (fprops->sw_format) {
  case XMA_YUV420P_FMT_TYPE:
  case XMA_YUV420P10LE_FMT_TYPE:
  case XMA_NV12_FMT_TYPE:
...
/**
 * Write the xma frame to a file
 * @param handle The handle to the device
 * @param log The handle to the logging
 * @param out_xframe The xma frame containing output buffers
 * @param out_f The opened output file to which output is written
 */
int xlnx_utils_write_xframe(XmaHandle handle, XmaLogHandle log, XmaFrame* out_xframe, FILE* out_f) {
  if (out_xframe == NULL) {
    xma_logmsg(log, XMA_ERROR_LOG, XLNX_APP_UTILS_MODULE, "Tried to write NULL xma frame to file!\n");
    return XMA_ERROR;
  }
  if (out_xframe->data[0].buffer_type != XMA_HOST_BUFFER_TYPE) {
    xma_logmsg(log, XMA_ERROR_LOG, XLNX_APP_UTILS_MODULE, "Tried to non host buffer to file!\n");
    return XMA_ERROR;
  }
  XmaFrameProperties* fprops = &out_xframe->frame_props;
  for (int plane_id = 0; plane_id < xma_frame_planes_get(handle, fprops); plane_id++) {
    uint8_t* p               = out_xframe->data[plane_id].host;
    int      valid_line_size = xlnx_utils_get_valid_line_size(log, fprops, plane_id);
    if (valid_line_size == XMA_ERROR) {
      return XMA_ERROR;
    }
    int plane_height    = xlnx_utils_get_valid_lines(handle, fprops, plane_id);
    int total_line_size = fprops->linesize[plane_id];
    for (int h = 0; h < plane_height; h++) {
      fwrite(p, valid_line_size, 1, out_f);
      p += total_line_size;
    }
  }
  xma_frame_free(out_xframe);
  fflush(out_f);
  return XMA_APP_SUCCESS;
}

/**
 * Read the next frame into the out buffers.
 * @param handle The handle to the device
 * @param log The handle to the logging
 * @param frame The frame to read in to
 * @param in_fp Opened input file
 * @return XMA_APP_SUCCESS on success, XMA_APP_EOF on eof.
 */
int32_t xlnx_utils_read_frame(XmaHandle handle, XmaLogHandle log, XmaFrame* frame, FILE* in_fp) {
  int32_t read_size  = 0;
  int32_t frame_size = 0;

  XmaFrameProperties* fprops = &frame->frame_props;
  for (int plane_id = 0; plane_id < xma_frame_planes_get(handle, fprops); plane_id++) {
    uint8_t* p               = frame->data[plane_id].host;
    int      valid_line_size = xlnx_utils_get_valid_line_size(log, fprops, plane_id);
    if (valid_line_size == XMA_ERROR) {
      return XMA_ERROR;
    }
    int plane_height    = xlnx_utils_get_valid_lines(handle, fprops, plane_id);
    int total_line_size = fprops->linesize[plane_id];
    frame_size += valid_line_size * plane_height;
    for (int h = 0; h < plane_height; h++) {
      read_size += fread(p, 1, valid_line_size, in_fp);
      p += total_line_size;
    }
  }
