 ...
 */
static int32_t decode_hevc_profile_tier_level(XlnxDecGetBits* get_bits, XlnxHEVCSeqParamSet* sps) {
  /* profile_space */
...
static int32_t hevc_scal_list_data(XlnxDecGetBits* get_bits) {
  ...
  for (size_id = 0; size_id < 4; size_id++) {
    ...
    for (matrix_id = 0; matrix_id < 6; matrix_id += ((size_id == 3) ? 3 : 1)) {
      uint8_t scal_list_pred_mode_flag = xlnx_dec_get_bits_byte(get_bits, 1);
 ...
 * @return XMA_APP_SUCCESS on success
 */
static int32_t hevc_decode_short_term_rps(XlnxDecGetBits* get_bits, XlnxHEVCShortTermRPS* rps, XlnxHEVCSeqParamSet* sps) {
  int32_t k  = 0;
  int32_t k0 = 0;
  int32_t delta_poc;
  ...
  if (rps_predict) {
    ...
    uint32_t bs_num_negative_pics = rps->num_negative_pics >> 1;
    if (bs_num_negative_pics != 0) {
      int32_t used;
      k = rps->num_negative_pics - 1;
      for (uint32_t i = 0; i < bs_num_negative_pics; i++) {
        delta_poc         = rps->delta_poc[i];
        used              = rps->used[i];
    ...
    if (rps->num_delta_pocs) {
      ...
      for (uint32_t i = 0; i < nb_positive_pics; i++) {
        ...
        rps->used[rps->num_negative_pics + i]      = xlnx_dec_get_bits_byte(get_bits, 1);
      }
    }
  }
 ...
 */
static int32_t parse_hevc_sps(XmaLogHandle log, int32_t file, XlnxDecBuffer* buffer, XlnxDecFrameParseData* parse_data, int32_t start_offset) {
  int32_t endoffset;
  int32_t ret = xlnx_dec_find_next_start_code(log, file, buffer, start_offset + 1, 0, &endoffset);
  if (ret == XMA_APP_EOF)
  ...
  ret = xlnx_dec_convert_to_rbsp(log, buffer, start_offset, endoffset, &nbuffer);
  if (ret == XMA_APP_ERROR)
    return XMA_APP_ERROR;
  ...
  if (chroma_format_idc == 3) {
    uint8_t separate_colour_plane_flag = xlnx_dec_get_bits_byte(&getbits, 1);
  ...
  if (pic_conformance_flag) {
    ...
    uint32_t output_window_left_offset, output_window_right_offset, output_window_top_offset, output_window_bottom_offset;
    /* output_window_left_offset */
    output_window_left_offset = xlnx_dec_get_bits_unsigned_eg(&getbits) * horiz_mult;
    /* output_window_right_offset */
    output_window_right_offset = xlnx_dec_get_bits_unsigned_eg(&getbits) * horiz_mult;
    ...
    output_window_bottom_offset = xlnx_dec_get_bits_unsigned_eg(&getbits) * vert_mult;
    parse_data->width           = coded_width - (output_window_left_offset + output_window_right_offset);
    parse_data->height          = coded_height - (output_window_top_offset + output_window_bottom_offset);
  }
  ...
  uint8_t long_term_ref_pics_present_flag = xlnx_dec_get_bits_byte(&getbits, 1);
  if (long_term_ref_pics_present_flag) {
    uint32_t num_long_term_ref_pics_sps = xlnx_dec_get_bits_unsigned_eg(&getbits);
  ...
  if (vui_present) {
    ...
    uint8_t overscan_info_present_flag = xlnx_dec_get_bits_byte(&getbits, 1);
    if (overscan_info_present_flag) {
      /* overscan_appropriate_flag */
      xlnx_dec_get_bits_byte(&getbits, 1);
    }

    uint8_t video_signal_type_present_flag = xlnx_dec_get_bits_byte(&getbits, 1);
    if (video_signal_type_present_flag) {
      /* video_format */
      xlnx_dec_get_bits_byte(&getbits, 3);
      /* video_full_range_flag */
      xlnx_dec_get_bits_byte(&getbits, 1);
      uint8_t colour_description_present_flag = xlnx_dec_get_bits_byte(&getbits, 1);
    ...
    uint8_t chroma_loc_info_present_flag = xlnx_dec_get_bits_byte(&getbits, 1);
    ...
    uint8_t default_display_window_flag = xlnx_dec_get_bits_byte(&getbits, 1);
    if (default_display_window_flag) {
      int32_t vert_mult          = 1 + (chroma_format_idc < 2);
      int32_t horiz_mult         = 1 + (chroma_format_idc < 3);
      def_disp_win_left_offset   = xlnx_dec_get_bits_unsigned_eg(&getbits) * horiz_mult;
      def_disp_win_right_offset  = xlnx_dec_get_bits_unsigned_eg(&getbits) * horiz_mult;
      def_disp_win_top_offset    = xlnx_dec_get_bits_unsigned_eg(&getbits) * vert_mult;
      def_disp_win_bottom_offset = xlnx_dec_get_bits_unsigned_eg(&getbits) * vert_mult;
      parse_data->width          = coded_width - (def_disp_win_left_offset + def_disp_win_right_offset);
      parse_data->height         = coded_height - (def_disp_win_top_offset + def_disp_win_bottom_offset);
    }

    uint8_t vui_timing_info_present_flag = xlnx_dec_get_bits_byte(&getbits, 1);
 ...
 */
int32_t xlnx_dec_parse_next_h265_au(XmaLogHandle log, int32_t file, XlnxDecBuffer* buffer, XlnxDecFrameParseData* parse_data, bool is_first_au, int32_t* ret_offset) {
  int32_t startoffset = 0;
  int32_t hasslice    = 0;
  while (true) {
    int32_t endoffset = 0;
    int32_t ret       = xlnx_dec_find_next_start_code(log, file, buffer, startoffset, is_first_au, &endoffset);
    if (ret == XMA_APP_ERROR) {
      return XMA_APP_ERROR;
    }
    ...
    if (buffer->size <= (uint32_t) (endoffset + 6)) {
      ret = xlnx_dec_get_in_buf(log, file, buffer, 6);
      if (ret == XMA_APP_ERROR)
        return XMA_APP_ERROR;
      if (ret == XMA_APP_EOF) {
        *ret_offset = buffer->size;
    ...
    if (nalutype == 33)
      ret = parse_hevc_sps(log, file, buffer, parse_data, endoffset);
    if (ret == XMA_APP_ERROR) {
      return XMA_APP_ERROR;
    }
    if ((nalutype >= 32 && nalutype <= 35) || nalutype == 39 || (nalutype >= 41 && nalutype <= 44) || (nalutype >= 48 && nalutype <= 55)) {
      if (hasslice) {
        *ret_offset = endoffset;
        return XMA_APP_SUCCESS;
      }
    } else if (nalutype <= 9 || (nalutype >= 16 && nalutype <= 21)) {
      int32_t first_slice_segment_in_pic_flag = buffer->data[endoffset + 5] >> 7;
