...
typedef struct XlnxDecoderCtx {
  ...
  XmaLogHandle          log;
  XmaHandle             handle;
  XmaSessionHandle      download_session;
  XmaFilterProperties   xma_download_props;
  XmaFrameProperties    host_frame_props;
 ...
 * @return XMA_APP_SUCCESS on success XMA_APP_ERROR on error
 */
int32_t xlnx_dec_create_decoder_context(char* input_file_name, long loop_count, XlnxDecoderProperties dec_params, XlnxDecoderCtx* ctx);
