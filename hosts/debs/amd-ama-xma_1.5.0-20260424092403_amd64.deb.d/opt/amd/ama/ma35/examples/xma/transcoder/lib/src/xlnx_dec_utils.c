 ...
 */
void xlnx_dec_init_get_bits(XlnxDecGetBits* getbits, const uint8_t* start, const uint8_t* end) {
  getbits->start        = start;
  getbits->end          = end;
...
uint8_t xlnx_dec_get_bits_byte(XlnxDecGetBits* getbits, uint8_t bits) {
  ...
  while (bits > 0) {
    ...
    while (getbits->offset_bits < 8) {
      ret = (ret << 1) | ((getbits->start[getbits->offset_bytes] >> (7 - getbits->offset_bits)) & 0x01);
      getbits->offset_bits++;
      bits--;
      if (bits == 0) {
...
uint32_t xlnx_dec_get_bits_unsigned_eg(XlnxDecGetBits* getbits) {
  ...
  while (leadingzerobits != 0) {
    if (leadingzerobits > 8) {
      ...
      remain          = (remain << leadingzerobits) + xlnx_dec_get_bits_byte(getbits, leadingzerobits);
      leadingzerobits = 0;
    }
  }
 ...
 */
int32_t xlnx_dec_get_in_buf(int32_t file, XlnxDecBuffer* buffer, const uint32_t min_size) {
  if ((buffer->size + min_size) > buffer->allocated) {
    unsigned char* newdata = malloc(buffer->size + min_size);
    if (!newdata) {
      fprintf(stderr, "Malloc error of size %d.\n", buffer->size + min_size);
 ...
 * allocation error, or DEC_INPUT_EOF if the end of the input file is reached
 */
int xlnx_dec_find_next_start_code(int file, XlnxDecBuffer* buffer, int offset, int* retoffset) {
 ...
 */
int xlnx_dec_convert_to_rbsp(XlnxDecBuffer* buffer, int startoffset, int endoffset, XlnxDecBuffer* newbuffer) {
