 ...
 */
static int32_t xlnx_dec_hevc_profile_tier_level(XlnxDecGetBits* getbits, XlnxHevcSeqParamSet* sps) {
  /* profile_space */
...
static int32_t xlnx_dec_hevc_scal_list_data(XlnxDecGetBits* getbits) {
  ...
  for (size_id = 0; size_id < 4; size_id++) {
    ...
    for (matrix_id = 0; matrix_id < 6; matrix_id += ((size_id == 3) ? 3 : 1)) {
      uint8_t scal_list_pred_mode_flag = xlnx_dec_get_bits_byte(getbits, 1);
 ...
 * @return XMA_APP_SUCCESS on success
 */
static int32_t xlnx_dec_hevc_short_term_rps(XlnxDecGetBits* getbits, XlnxHevcShortRps* rps, XlnxHevcSeqParamSet* sps) {
  int32_t i;
  int32_t k  = 0;
  int32_t k0 = 0;
  int32_t delta_poc;
  ...
  if (rps_predict) {
    ...
    if ((rps->num_negative_pics >> 1) != 0) {
      ...
      for (uint32_t r = 0; r < rps->num_negative_pics >> 1; r++) {
        delta_poc         = rps->delta_poc[r];
        used              = rps->used[r];
        rps->delta_poc[r] = rps->delta_poc[k];
        rps->used[r]      = rps->used[k];
        rps->delta_poc[k] = delta_poc;
        rps->used[k]      = used;
        k--;
      }
    ...
    if (rps->num_delta_pocs) {
      prev = 0;
      for (uint32_t r = 0; r < rps->num_negative_pics; r++) {
        delta_poc = xlnx_dec_get_bits_unsigned_eg(getbits) + 1;
        prev -= delta_poc;
        rps->delta_poc[r] = prev;
        rps->used[r]      = xlnx_dec_get_bits_byte(getbits, 1);
      }
      prev = 0;
      for (uint32_t r = 0; r < nb_positive_pics; r++) {
        delta_poc = xlnx_dec_get_bits_unsigned_eg(getbits) + 1;
        prev += delta_poc;
        rps->delta_poc[rps->num_negative_pics + r] = prev;
        rps->used[rps->num_negative_pics + r]      = xlnx_dec_get_bits_byte(getbits, 1);
      }
    }
  }
 ...
 */
static int32_t xlnx_dec_parse_hevc_sps(int32_t file, XlnxDecBuffer* dec_buffer, XlnxDecFrameData* parsedata, int32_t startoffset) {
  int32_t endoffset;
  int32_t ret = xlnx_dec_find_next_start_code(file, dec_buffer, startoffset + 1, &endoffset);
  ...
  if (pic_conformance_flag) {
    ...
    parsedata->width  = coded_width - (output_window_left_offset + output_window_right_offset);
    parsedata->height = coded_height - (output_window_top_offset + output_window_bottom_offset);
  }
  ...
  if (long_term_ref_pics_present_flag) {
    ...
    for (uint32_t k = 0; k < num_long_term_ref_pics_sps; k++) {
  ...
  if (vui_present) {
    ...
    if (default_display_window_flag) {
      ...
      parsedata->width           = coded_width - (def_disp_win_left_offset + def_disp_win_right_offset);
      parsedata->height          = coded_height - (def_disp_win_top_offset + def_disp_win_bottom_offset);
    }
 ...
 */
int32_t xlnx_dec_parse_hevc_au(int32_t file, XlnxDecBuffer* dec_buffer, XlnxDecFrameData* parsedata, int32_t* retoffset) {
  ...
  while (true) {
    ...
    if (dec_buffer->size <= (uint32_t) (endoffset + 6)) {
      ret = xlnx_dec_get_in_buf(file, dec_buffer, 6);
      if (ret == XMA_APP_ERROR)
        return XMA_APP_ERROR;
      if (ret == DEC_INPUT_EOF) {
        *retoffset = dec_buffer->size;
    ...
    if ((nalutype >= 32 && nalutype <= 35) || nalutype == 39 || (nalutype >= 41 && nalutype <= 44) || (nalutype >= 48 && nalutype <= 55)) {
      ...
      int32_t first_slice_segment_in_pic_flag = dec_buffer->data[endoffset + 5] >> 7;
