 ...
 */
void xlnx_dec_init_get_bits(XlnxDecGetBits* get_bits, const uint8_t* start, const uint8_t* end) {
...
uint8_t xlnx_dec_get_bits_byte(XlnxDecGetBits* get_bits, uint8_t bits) {
  ...
  while (bits > 0) {
    ...
    while (get_bits->offset_bits < 8) {
      ret = (ret << 1) | ((get_bits->start[get_bits->offset_bytes] >> (7 - get_bits->offset_bits)) & 0x01);
...
uint32_t xlnx_dec_get_bits_unsigned_eg(XlnxDecGetBits* get_bits) {
  ...
  while (leading_zero_bits != 0) {
    if (leading_zero_bits > 8) {
      ...
      remain            = (remain << leading_zero_bits) + xlnx_dec_get_bits_byte(get_bits, leading_zero_bits);
      leading_zero_bits = 0;
    }
  }
 ...
 */
int32_t xlnx_dec_get_in_buf(XmaLogHandle log, int32_t file, XlnxDecBuffer* buffer, const uint32_t min_size) {
  if ((buffer->size + min_size) > buffer->allocated) {
    void* newdata = NULL;
    if (posix_memalign(&newdata, BUFFER_ALLOC_ALIGN, buffer->size + min_size) != 0) {
      xma_logmsg(log, XMA_ERROR_LOG, XLNX_DEC_APP_MODULE, "Unable to allocate input data of size %d.\n", buffer->size + min_size);
  ...
  do {
    int32_t dataread = read(file, (buffer->data + buffer->size), (buffer->allocated - buffer->size));
 ...
 * @param is_first_au: If this is the first access, the start code should be at
 * the start, otherwise it is the wrong file type.
 * @param ret_offset: returns the location of the start of the next start code
 * @param log: xma log handle
 * @return XMA_APP_SUCCESS on success, XMA_APP_ERROR on memory
 * allocation error or failure to find start code, or XMA_APP_EOF if the end
 * of the input file is reached
 */
int xlnx_dec_find_next_start_code(XmaLogHandle log, int file, XlnxDecBuffer* buffer, int offset, bool is_first_au, int* ret_offset) {
  uint8_t* pt  = buffer->data + offset;
  uint8_t* end = buffer->data + buffer->size - 3;
  while (true) {
    if (pt > end) {
      int ret = xlnx_dec_get_in_buf(log, file, buffer, 4096);
    ...
    if (is_first_au && pt - buffer->data > 1) {
      return XMA_APP_ERROR;
    }
    pt++;
  }
 ...
 */
int xlnx_dec_convert_to_rbsp(XmaLogHandle log, XlnxDecBuffer* buffer, int start_offset, int end_offset, XlnxDecBuffer* new_buffer) {
  new_buffer->allocated = end_offset - start_offset;
  if (posix_memalign((void**) &new_buffer->data, BUFFER_ALLOC_ALIGN, new_buffer->allocated) != 0) {
    xma_logmsg(log, XMA_ERROR_LOG, XLNX_DEC_APP_MODULE, "Unable to allocate input data.\n");
    return XMA_APP_ERROR;
  }
  new_buffer->size = 0;
  int      state   = 0;
