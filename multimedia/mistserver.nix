{ config, lib, pkgs, ... }:
{
  ...
  systemd.services."gstreamer-stream" = {
    ...
    serviceConfig = {
      ...
      TasksMax = "infinity";
      TimeoutStopSec = 8;
      ExecStart = "${lib.getExe pkgs.bash} -c \"${pkgs.gst_all_1.gstreamer}/bin/gst-launch-1.0 v4l2src device=/dev/video0 ! videoconvert ! x264enc ! flvmux ! rtmpsink location=rtmp://127.0.0.1/live/$(${pkgs.coreutils-full}/bin/cat ${config.sops.secrets."rtmp_key".path})\"";
      User = "root";
      Group = "root";
