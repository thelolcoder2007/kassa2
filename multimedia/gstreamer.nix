...
let
  gstreamer-sh = pkgs.writeShellScriptBin "gstreamer.sh" ''
    ${pkgs.gst_all_1.gstreamer}/bin/gst-launch-1.0 \
        v4l2src device=/dev/video0 ! videoconvert ! "video/x-raw, width=3840, height=2160, framerate=60/1" \
        ! queue ! x264enc cabac=1 bframes=2 ref=1 ! "video/x-h264,profile=main" ! flvmux streamable=true name=muxer \
        ! rtmpsink location="rtmp://127.0.0.1/live/$(${pkgs.coreutils-full}/bin/cat ${
